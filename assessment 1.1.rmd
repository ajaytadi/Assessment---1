...
```{r factor}
class(final$restaurant)

final2 <- as_factor(final$restaurant)
final2 <- as_factor(final$calories)
...
final2 <- final %>% mutate(calories_type)
...
final2 %>% ggplot(aes(calories_type,restaurant,)) +
  geom_col() +
   coord_flip() +
...
```{r adding}
final2 <- final2 %>% mutate(trans_fat_percent = trans_fat/total_fat * 100)
...
```{r plots}
  ...
  ggplot(final2, aes(trans_fat_percent,restaurant)) + 
  geom_col() +
   coord_flip() +
  facet_wrap(~restaurant) +
  theme_linedraw()
